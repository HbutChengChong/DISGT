...
class CONSTRUCT_SUBGRAPH(object):
    def __init__(self, data_name, Subspace, split_x, split_y, path_save_graph, path_save_Sub, deltas, lambdas, p, threshold):
        ...
        self.Subspace_list_AedFCM = []  # for AedFCM cluster
        self.SubGraph_list_AedFCM = []
        ...
        self.AedFCM()
        self.Construct_Graph_set_AedFCM()
        self.save_graph_AedFCM()
    ...
    def AedFCM(self):
        v, u, _, _ = FSP(self.AedFCM_data, self.Subspace, delta=self.deltas, lambdas=self.lambdas, p=self.p).Fuzzy_Subspace_Partition()
        # interact
        for i in range(self.Subspace):
            labels = np.where(u[i, :] > self.threshold)[0]
            self.Subspace_list_AedFCM.append(labels)
        ...
        #     self.Subspace_list_AedFCM.append(members)

        return self.Subspace_list_AedFCM
    ...
    def Construct_SubGraph_AedFCM(self, Subgraph_NO):
        Subset_Band = self.transpose_matrix.take(self.Subspace_list_AedFCM[Subgraph_NO], axis=0)
        ...
        g.ndata['NO.'] = torch.Tensor(self.Subspace_list_AedFCM[Subgraph_NO].reshape(sub_node_num, 1)).to(device)
        for i in range(sub_node_num):
            for j in range(sub_node_num):
                if i == j:
                    ...
                    a = self.Subspace_list_AedFCM[Subgraph_NO][i]
                    b = self.Subspace_list_AedFCM[Subgraph_NO][j]
        ...
        return g
    def Construct_Graph_set_AedFCM(self):
        subgraph_num = self.Subspace
        for i in tqdm(range(subgraph_num)):
            g = self.Construct_SubGraph_AedFCM(i)
            self.SubGraph_list_AedFCM.append(g)
        return self.SubGraph_list_AedFCM
    def save_graph_AedFCM(self):
        print("Save Subgraph...")
